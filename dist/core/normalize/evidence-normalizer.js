...
export function normalizeEvidenceHealth(params) {
    ...
    for (const feat of features) {
        ...
        // Aggregate values
        const commandsRecorded = [];
        const exitCodesRecorded = [];
        let testCounts;
        ...
        if (reviewEvidence) {
            if (reviewEvidence.commands)
                commandsRecorded.push(...reviewEvidence.commands);
            if (reviewEvidence.exitCodes)
                exitCodesRecorded.push(...reviewEvidence.exitCodes);
            if (reviewEvidence.testCounts)
                testCounts = reviewEvidence.testCounts;
            if (reviewEvidence.approver) {
                approvalRecorded = true;
                approver = reviewEvidence.approver;
            }
            if (reviewEvidence.reviewer)
                reviewer = reviewEvidence.reviewer;
            if (reviewEvidence.rollbackForwardFixNotes)
                rollbackNotesExists = true;
            if (reviewEvidence.residualRisks)
                residualRisksExists = true;
            if (reviewEvidence.productDoctorNotes)
                productDoctorNotesExists = true;
            if (reviewEvidence.openApiNotes)
                openApiNotesExists = true;
        }
        // Accumulate other evidence items
        for (const ev of featEvidence) {
            if (ev === reviewEvidence)
                continue;
            if (ev.commands)
                commandsRecorded.push(...ev.commands);
            if (ev.exitCodes)
                exitCodesRecorded.push(...ev.exitCodes);
            if (ev.testCounts && !testCounts)
                testCounts = ev.testCounts;
        }
        // Missing referenced evidence files
        // Let's look for markdown links or file paths in tasks/phase-exit files that don't exist
        const missingReferencedEvidence = [];
        ...
        if (phaseExitExists)
            evidenceHealthScore += 15;
        if (commandsRecorded.length > 0)
            evidenceHealthScore += 15;
        if (exitCodesRecorded.length > 0 && exitCodesRecorded.every(c => c === 0))
            evidenceHealthScore += 15;
        if (testCounts && testCounts.total > 0)
            evidenceHealthScore += 15;
        if (approvalRecorded)
            evidenceHealthScore += 15;
        if (rollbackNotesExists)
            evidenceHealthScore += 10;
        if (residualRisksExists)
            evidenceHealthScore += 5;
        if (productDoctorNotesExists || openApiNotesExists)
            evidenceHealthScore += 10;
        healthList.push({
