...
export function normalizeFeatures(params) {
    ...
    for (const [featNum, featData] of featureMap.entries()) {
        ...
        if (exitEvidence) {
            if (exitEvidence.commands && exitEvidence.commands.length > 0)
                evidenceHealthScore += 15;
            ...
            if (exitEvidence.testCounts && exitEvidence.testCounts.total > 0)
                evidenceHealthScore += 15;
            if (exitEvidence.approver)
                evidenceHealthScore += 15;
            if (exitEvidence.rollbackForwardFixNotes)
                evidenceHealthScore += 10;
            if (exitEvidence.residualRisks)
                evidenceHealthScore += 5;
            if (exitEvidence.productDoctorNotes || exitEvidence.openApiNotes)
                evidenceHealthScore += 10;
        }
        // Derived next action
