...
export class GovernanceLedgerAdapter {
    ...
    parse(input) {
        ...
        // E.g., Heading matches: ## P1: Engine Complete (OPEN)
        const gateHeadingRegex = /##?\s*(P[0-4])\s*[:\-]?\s*([^(\n]+)(?:\(([^)]+)\))?/i;
        for (const sec of flatSections) {
            const match = sec.title.match(gateHeadingRegex);
            if (match) {
                const gateId = match[1].toUpperCase();
                const gateTitle = match[2].trim();
                const gateStatus = (match[3] || 'OPEN').trim().toUpperCase();
                ...
                for (const table of tables) {
                    ...
                    const featureColIdx = table.headers.findIndex(h => /feature|id|child/i.test(h));
                    ...
                    if (featureColIdx !== -1) {
                        for (const row of table.rows) {
                            const featText = row.cells[featureColIdx];
                            const featMatch = featText.match(/\b(\d{3})\b/);
                            if (featMatch) {
                                const featNum = parseInt(featMatch[1], 10);
                                ...
                                const isVerified = statusText.includes('VERIFIED') || statusText.includes('CLOSED') || statusText.includes('DONE') || statusText.includes('COMPLETE');
                                if (isVerified) {
        ...
        if (gates.length > 0)
            entities.gates = gates;
        if (activities.length > 0)
            entities.activities = activities;
        if (risks.length > 0)
            entities.risks = risks;
        return {
            adapterId: this.id,
