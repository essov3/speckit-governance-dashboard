...
import { Adapter, ParsedFragment, FeatureFragment, TaskFragment, GateFragment, DecisionFragment, CoverageFragment, EvidenceFragment, ActivityFragment, RiskFragment, ContractFragment } from '../adapters/adapter-types.ts';
...
import { normalizeGates, Gate } from '../normalize/gate-normalizer.ts';
import { normalizeDecisions, Decision } from '../normalize/decision-normalizer.ts';
import { normalizeEvidenceHealth, EvidenceHealth } from '../normalize/evidence-normalizer.ts';
import { normalizeCoverage, CoverageRow } from '../normalize/coverage-normalizer.ts';
import { normalizeActivities, ActivityEvent } from '../normalize/activity-normalizer.ts';
...
import { ValidationContext } from '../validate/validator-types.ts';
import { ProjectStatusSnapshot } from './snapshot-schema.ts';
 ...
 */
export async function buildSnapshot(options: BuildSnapshotOptions): Promise<ProjectStatusSnapshot> {
  ...
  const featuresList: FeatureFragment[] = [];
  const tasksList: TaskFragment[] = [];
  const gatesList: GateFragment[] = [];
  const decisionsList: DecisionFragment[] = [];
  const coverageList: CoverageFragment[] = [];
  const evidenceList: EvidenceFragment[] = [];
  const contractsList: ContractFragment[] = [];
  const activitiesList: ActivityFragment[] = [];
  const risksList: RiskFragment[] = [];

  const taskPathMap = new Map<string, string>(); // featureNum -> tasks file relative path
  const decisionPathMap = new Map<string, string>(); // decisionId -> file relative path
  const activityPathMap = new Map<string, string>(); // key -> file relative path

  for (const frag of parsedFragments) {
    const p = frag.artifactPath;
    if (frag.entities.features) featuresList.push(...frag.entities.features);
    ...
    if (frag.entities.gates) gatesList.push(...frag.entities.gates);
    ...
    if (frag.entities.coverageRows) coverageList.push(...frag.entities.coverageRows);
    if (frag.entities.evidenceItems) evidenceList.push(...frag.entities.evidenceItems);
    if (frag.entities.contracts) contractsList.push(...frag.entities.contracts);
    ...
    if (frag.entities.risks) risksList.push(...frag.entities.risks);
  }
  ...
  const highestSeverity: 'none' | 'info' | 'warning' | 'error' = 
    allErrors.length > 0 ? 'error' : allWarnings.length > 0 ? 'warning' : 'none';
  ...
  let constitutionSummary: any = undefined;
