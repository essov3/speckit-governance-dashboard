...
export interface ExtractedLinks {
  ...
  frIds: string[];
  taskIds: string[];
  featureIds: string[]; // E.g., '090', '087'
  decisionIds: string[]; // E.g., 'D1', 'D-2'
  gateIds: string[]; // E.g., 'P0', 'P1'
  filePaths: string[];
}
 ...
 */
export function extractLinksAndIds(text: string): ExtractedLinks {
...
export function resolveReferencedPath(params: {
  ...
  artifactPath: string; // Absolute path to the source file containing the reference
  projectRoot: string; // Absolute path to the project root
  featureDir?: string; // Absolute path to the feature directory (if any)
}): string | null {
