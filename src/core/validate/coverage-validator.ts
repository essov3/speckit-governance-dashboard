import { CoverageRow } from '../normalize/coverage-normalizer.ts';
...
export function validateCoverage(
  ...
  for (const contractPath of contractPaths) {
    ...
    if (isOpenApi) {
      ...
      const isReferenced = coverageRows.some(row => 
        row.evidenceLinks.some(link => contractPath.includes(link)) ||
